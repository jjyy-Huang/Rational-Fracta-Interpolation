...
//    cv::Mat img_H;
    ...
    img_read_GT = cv::imread("../img/GT/0.bmp");
    if ( isLRImageSet == true) {
        img_read_LR = cv::imread("../img/LR/0.bmp");
...
//    core upsampling function
    cv::Mat img_SR_y = core(img_LR_y, img_read_GT.rows, img_read_GT.cols);
    clock_t end   = clock();
...
//    cv::imwrite("../img/output/girl.bmp", img_SR_bgr);
//
    return 0;
}
